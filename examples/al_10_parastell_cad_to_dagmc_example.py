...
def call_with_docker(*command: str) -> Callable[[Callable[..., None]], Callable[..., None]]:  # 末尾はコンテナ内の python。ホストの sys.executable はコンテナから見えない
	import functools
	import inspect
	import subprocess
	def decorator(func: Callable[..., None]) -> Callable[..., None]:
		script = pathlib.Path(inspect.getfile(func)).resolve()
		@functools.wraps(func)
		def wrapper(*args: Any, **kwargs: Any) -> None:
			subprocess.run([*command, "-c",
				f'import sys; sys.path.insert(0, "/work/{script.parent.relative_to(pathlib.Path.cwd()).as_posix()}");'  # /work はカレント、スクリプトはその下
				f' from {script.stem} import {func.__name__}; {func.__name__}.__wrapped__()'  # 素の関数を呼びデコレータの再入を避ける
			], check=True)
		return wrapper
	return decorator
...
@call_with_docker(
	"docker", "run", "--rm", "-i",
	"-e", "PATH=/opt/conda/envs/parastell_env/bin:/usr/bin:/bin",  # 末尾の python はこの PATH で解決される
	"-v", f"{pathlib.Path.cwd()}:/work",
	"-w", "/work",
	"ghcr.io/svalinn/parastell-ci",
	"python",
)
def main_parastell(
...
) -> None:  # ParaStell で層ごとの STEP と四面体線源メッシュを書く。コンテナの中だけで走り、輸送は main が続ける
...
def build(wout: pathlib.Path, coils: pathlib.Path, out: pathlib.Path, parameters: dict[str, Any]) -> tuple[dict[str, float], dict[str, Any]]:  # 1 周期分の in-vessel build・マグネット・四面体線源メッシュを作り、層ごとの STEP と体積を返す
	import cadquery as cq  # コンテナにしか無いので関数内で import し、ホスト側がこのモジュールを import できるようにする
...
def materials() -> list[openmc.Material]:  # ParaStell 論文 Table 1/2 (ARIES-CS の DCLL) の均質化材料。name が DAGMC の材料タグと一致する
	def constituent_material(name: str, density: float, atoms: dict[str, float], percent: str = "ao", enrichment_isotopes: dict[str, float] | None = None) -> openmc.Material:  # enrichment_isotopes は {"Li6": 90.0} のように同位体名 → 存在比 (percent と同じ単位)
...
def dagmc(layers: list[tuple[pathlib.Path, str, int, float]], h5m: pathlib.Path, tolerance: float, angular_tolerance: float) -> pathlib.Path:  # 層ごとの STEP を 1 つの DAGMC にする。材料タグは層名で chamber は void。STEP は cm なので scale は 1
	...
	cad.export_dagmc_h5m_file(filename=str(h5m), meshing_backend="cadquery", tolerance=tolerance, angular_tolerance=angular_tolerance, imprint=False)  # gmsh はホストで面を出さない (file 経由は 0 面、in memory はアクセス違反)。imprint は 24 ソリッドで 10 分でも終わらないので切り、共有面のぶんは lost で数える
	return h5m
...
def run(
...
) -> dict[str, Any]:  # 90° 扇形の DAGMC を CSG の回転周期平面で囲み、TBR・層別核加熱 [eV/中性子]・発熱マップを返す
	...
	plane_0 = openmc.YPlane(0.0, boundary_type="periodic")  # DAGMC は periodic 未対応なので φ=0 (y=0) と φ=90° (x=0) を CSG で回転周期にする
	plane_90 = openmc.XPlane(0.0, boundary_type="periodic")
	plane_0.periodic_surface = plane_90
	cell = openmc.Cell(region=dagmc_universe.bounding_region(padding_distance=10.0) & +plane_0 & +plane_90, fill=dagmc_universe)  # bbox の外側 4 面は真空
	...
	settings.max_lost_particles = max(10, particles * batches // 1000)  # 周期平面と切断面が一致して lost が出うるが、打ち切らず数を報告する
	model = openmc.Model(
...
def report(**kwargs: Any) -> str:
	return """# ParaStell 形状で TBR と核加熱を出す (al_10)
...
処理は 2 段に分かれる。`main_parastell` は `ghcr.io/svalinn/parastell-ci` コンテナの中で ParaStell を動かし、
層ごとの STEP と四面体線源メッシュを書く。`main` はホストの openmc-anywhere でそれを読み、
cad_to_dagmc で DAGMC にして輸送する。コンテナの conda 版 OpenMC は光子輸送で落ちるので、
輸送をホストに置くことで光子込みの核加熱が取れる。
...
![ボクセル別のトリチウム収支。負 (青) はプラズマで DT 反応が T を燃やす密度、正 (赤) は LiPb で Li-6, Li-7 が T を作る密度。全体積の積分が (TBR − 1) × 線源強度になる]({tritium_png})
