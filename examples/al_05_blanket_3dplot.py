...
1. VMEC 自身の語彙である。`xm` / `xn` (poloidal / toroidal mode number) と同じ数え方であり、幾何カーネルの一層下が既にこの表現で動いている。読み手に説明が要らない。
2. 設計判断を決め打ちしない。W2 v0 のポロイダル単一パスと HELIAS の QTS 相当を同じデータ構造で並べられるので、どちらが正解かを主張せずに Pareto を出せる。
3. 拡張が値の追加で済む。W10 のスキャン軸 (本数・アスペクト比・厚み・FCI 伝導度) を足しても実数が数個増えるだけで、表現の作り直しにはならない。
