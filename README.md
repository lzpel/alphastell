...
Tritium balance per voxel: negative where DT burns tritium (the neutron source), positive where Li-6 and Li-7 breed it (`(n,Xt)` on the same mesh); the volume integral is (TBR - 1) times the source strength.
