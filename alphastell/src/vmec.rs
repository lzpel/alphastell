...
// upstream との diff が取れる状態を保つため、型名 (upstream: VmecData) 以外は変更していない。
...
/// # 名前の由来
///
/// (R, Z) を角度 (θ, φ) の Fourier 級数で書く表現は、核融合分野では **逆モーメント表現**
...
#[pyclass(name = "SurfaceRZFourier")]
#[repr(transparent)]
pub struct PySurfaceRZFourier(SurfaceRZFourier);

#[pymethods]
impl PySurfaceRZFourier {
	#[staticmethod]
	fn load(file: &Bound<'_, PyAny>) -> PyResult<Self> {
		...
		SurfaceRZFourier::load(std::io::Cursor::new(data))
			.map(Self)
	...
	fn point_normal(&self, phi: f64, theta: f64, s: f64, use_surface: bool) -> [[f64; 3]; 2] {
		self.0.interpolate(phi, theta, s, match use_surface {
			true => NormalKind::Surface,
			false => NormalKind::Planar,
		})
	}
}

pub fn module(m: &Bound<'_, PyModule>) -> PyResult<()> {
	m.add_class::<PySurfaceRZFourier>()?;
	Ok(())
}
...
mod tests {
	...
	/// 環境変数 `PATH_WMEC` が指す wout ファイルを読む。
	///
	/// - **未設定** → `None` を返し、呼び出し側のテストは早期 return する。
	...
	fn stellarator_symmetry() {
		...
		for &s in &[0.25, 0.5, 1.0] {
			for i in 0..17 {
				let phi = TAU * (i as f64) / 17.0;
				for j in 0..23 {
					let theta = TAU * (j as f64) / 23.0;
					let a = vmec.interpolate_rz(phi, theta, s);
					let b = vmec.interpolate_rz(-phi, -theta, s);
					max_dr = max_dr.max((a.r - b.r).abs());
					max_dz = max_dz.max((a.z + b.z).abs());
				}
			}
		}
		eprintln!("stellarator symmetry: max|ΔR|={max_dr:.3e}, max|ΔZ|={max_dz:.3e}");
		assert!(max_dr < 1e-12, "R(−φ,−θ) ≠ R(φ,θ): {max_dr:.3e}");
	...
	fn export_point_cloud_csv() {
		...
		let mut out = String::from(
			"s,i_phi,i_theta,phi,theta,R,Z,x,y,z,dR_dtheta,dZ_dtheta,dR_dphi,dZ_dphi\n",
		);
		let mut rows = 0usize;
		for &s in &surfaces {
			for i in 0..DIV_PHI {
				...
				for j in 0..DIV_THETA {
					let theta = TAU * (j as f64) / (DIV_THETA as f64);
					let p = vmec.interpolate_rz(phi, theta, s);
					out.push_str(&format!(
						"{s},{i},{j},{phi:.17e},{theta:.17e},{:.17e},{:.17e},\
					...
					assert!(
						p.r.is_finite() && p.z.is_finite(),
						"非有限値 at s={s}, φ={phi}, θ={theta}"
					);
					rows += 1;
				}
			}
