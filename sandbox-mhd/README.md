# sandbox-mhd — Docker + epotFoam による Hartmann 流の検証サンドボックス
...
リポジトリルートからは `make -C sandbox-mhd` で呼べる。
