...
起動できるようにする。利用側(sandbox-mhd-cadrum)から wmake・環境設定・マウント合成の
知識を消すのが目的。
...
| ソース配置 | epotFoam.C / createFields.H / Make/ を**このディレクトリにコピーして保持** | ディレクトリの完結性を優先(ユーザー方針)。原本は [sandbox-mhd/epotFoam](../sandbox-mhd/epotFoam/) と同一 — **ソルバーを変更したら両方を同期すること** |
