...
//! # このモジュールの API
//!
//! 1. [`load_vmec`] — netCDF ファイルを開いて [`VmecData`] を作る
//! 2. [`interp_coeffs_at_s`] — 目的の s における Fourier 係数を内挿で求める
//! 3. [`eval_rz`] — (θ, φ) を 1 つ指定して (R, Z) を計算する
//!
...
// ================================================================
// load_vmec — netCDF から VmecData を作る
// ================================================================
...
pub fn load_vmec(path: &Path) -> Result<VmecData> {
...
/// 補間が必要。本関数は **mnmax 個それぞれ**について NaturalSpline を作って
/// 目的の s で評価する。
///
/// 返り値の長さはそれぞれ mnmax。以後 `eval_rz` はこの係数を受けて (θ, φ) だけで
/// 波の和を取れる (つまり s は事前に「焼き込まれる」)。
pub fn interp_coeffs_at_s(vmec: &VmecData, s: f64) -> (Vec<f64>, Vec<f64>) {
