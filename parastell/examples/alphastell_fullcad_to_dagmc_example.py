...
取り込んでいる機能:
  [in-vessel]
    ...
    - 独自の 'chamber' エントリで SOL 厚みを toroidal/poloidal で変化させる
    - 全コンポーネントに mat_tag を明示
    - plasma_mat_tag / sol_mat_tag を明示 (Vacuum 以外のタグを付与)
    - num_ribs / num_rib_pts を既定より高解像に上げる
    - scale を明示指定 (m -> cm)
...
wall_s = 1.15  # > 1.0 -> SOL を分離可能に

uniform = np.ones((len(toroidal_angles), len(poloidal_angles)))

# SOL 厚み (chamber) を poloidal で揺らして "step-like" な厚み変化を近似
chamber_thickness = np.tile(
    np.array([3.0, 4.0, 5.0, 6.0, 5.0, 6.0, 5.0, 4.0, 3.0]),
    (len(toroidal_angles), 1),
)
